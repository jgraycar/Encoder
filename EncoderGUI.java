...
public class EncoderGUI {
    ...
    String fileName;
    JTextArea fileText;
    EncDecButtonsPanel encDecButtons;
    JButton encButt, decButt;
    boolean nameSet;
    ...
    public void go() {
        ...
        encDecButtons = new EncDecButtonsPanel();
        encDecButtons.add(encButt);
        encDecButtons.add(decButt);
        filePanel.add(encDecButtons);
    ...
    private void doAction() {
        ...
        args[2] = fileName;
        args[3] = currFile;
        int status = enc.parseArgs(args);
        if (status == 1) {
            popup("Error: NullPointerException while encrypting.");
        } else if (status == 2) {
            popup("Error: IOException while encrypting.");
        } else if (status == 3) {
    ...
    class FileButtonListener implements ActionListener {
        public void actionPerformed(ActionEvent event) {
            ...
            currFile = pressed.getText();
            BufferedReader file = Encoder.getAFile(currFile);
    ...
    class EncDecButtonsPanel extends JPanel {
        public void paintComponent(Graphics g) {
